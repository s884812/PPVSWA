...
<br>
<br>
欲刪除之帳號：<input type="text" name="data1"> 
<!--<p>新密碼：&nbsp; <input type="text" name="data2"></p>-->
<p>&nbsp;<input type="hidden" name="id" value="1"><input type="submit" class="btn btn-primary" name="Submit" value="送出"> 
</p>
</form> 
</body> 
</html>
...
$sql1 = "DELETE FROM `vpn`.`vpn` WHERE `vpn`.`name` = '$_POST[data1]'";   //刪除使用者
...
mysql_query($sql1, $link) or die ("無法刪除".mysql_error()); 
//mysql_query($sql2, $link) or die ("無法修改".mysql_error());
mysql_close($link);
if ($_POST[data1] = $_POST[data1]){
echo $_POST[data1];
echo '用戶刪除成功，別忘記要重新生成chap-secrets檔案哦！<br>';
}else{
echo "尚未輸入欲刪除之用戶名<br>";
}
}
